...
# dftab <- data.frame(
#   generation = c("P1", "P2", "F1", "F2", "BC1", "BC2"),
...
for (i in 1:nrow(traits)) {
  ...
  for (v in c("rawvar", "predvar")) {
    ...
    gmaAD <- data.frame(
      parameters = as.factor(c("mhat", "ahat", "dhat")),
      estimates = parhat,
      varcomp = diag(xdxinv),
      ...
      df = sum(n - 1),
      pval = round(
        2 * (1 - pt(abs(parhat / sqrt(diag(xdxinv))), df = sum(n - 1))),
        8
      )
    )
    ...
    gof <- as.matrix(
      pchisq(
        sum(as.vector(sqrt(sig2)^-1) * as.vector(mu - muhat)^2),
        df = 3,
        lower.tail = FALSE
      )
    )

    gof_list[[paste0(v, "_", trait)]] <- gof
    r2AD_list[[paste0(v, "_", trait)]] <- r2AD

    sq <- w^2 / diag(te)
    r2 <- ((w^2 / diag(te)) * 100) / sum(w^2 / diag(te))

    gmaAD <- data.frame(cbind(
      model = as.factor(round(r2AD, 3)),
      # kolmo = as.factor(round(kolmo, 4)),
      gmaAD,
      SQ = sq,
      R2 = r2
    ))
    ...
    gmaE <- data.frame(
      ...
      pval = round(
        2 *
          (1 -
            pt(
              abs((parhat %*% mu) / sqrt(parhat^2 %*% (sig2 / n))),
              df = degfree %*% (n - 1)
            )),
        8
      )
    ...
    #   cat(i, "[",
    #       scaling[i,"scau"] - sqrt(scaling[i,"scavar"]),
    #       "to",
    #       scaling[i,"scau"] + sqrt(scaling[i,"scavar"]),
    #       "] ","\n" , sep = " ")
    # }
  ...
  gma_list[[abrv]] <- bind_rows(gma)
}

raw <- do.call(rbind.data.frame, r2AD_list[startsWith(names(r2AD_list), "raw")])
pred <- do.call(
  rbind.data.frame,
  r2AD_list[startsWith(names(r2AD_list), "pred")]
)
r2AD <- do.call(rbind.data.frame, r2AD_list)
gof <- do.call(rbind.data.frame, gof_list)

delta <- pred |>
  bind_cols(raw, delta = pred - raw, .name_repair = "unique_quiet") |>
  rownames_to_column() |>
  setNames(c("trait", "pred", "raw", "delta")) |>
  mutate(trait = gsub("predvar_", "", trait))

r2AD <- do.call(rbind.data.frame, r2AD_list) |>
  bind_cols(gof, .name_repair = c("minimal")) |>
  setNames(c("r2", "pval")) |>
  rownames_to_column() |>
  separate(
    ...
    remove = TRUE,
    fill = "right"
  ) |> # Keep original "id" col
  right_join(variance, by = c("trait", "Dataset")) |>
  mutate(pval = round(pval, 4)) |>
  relocate(trait) |>
  relocate(c("r2", "pval"), .after = h2n)

delta_list <- list()

for (abrv in traits$abrv) {
  dgma <- gma_list[[abrv]]
...
analysis <- r2AD |>
  pivot_wider(names_from = Dataset, values_from = c(sig2env:pval)) |>
  full_join(traits, by = "trait") |>
  relocate(all_of(names(traits))) |>
  pivot_longer(cols = c(sig2env_rawvar:pval_predvar), names_to = "Dataset") |>
  separate(Dataset, sep = "_", into = c("var", "Dataset")) |>
  pivot_wider(names_from = Dataset, values_from = value) |>
  mutate(delta = predvar - rawvar)


# pivot_wider(names_from=var, values_from=c(rawvar:delta), names_vary="slowest")

write.csv(analysis, file = "./analysis.csv")

gma_pkg <- bind_rows(gma_list, .id = "abrv")
