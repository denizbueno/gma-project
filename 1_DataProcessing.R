...
df_irr <- read.csv(
  "Analise_de_Geracao-TomateAnao-REV00-DENIZ-PC - Uniformidade.csv",
  header = T
)
df_raw <- read.csv(
  "Analise_de_Geracao-TomateAnao-REV48-DENIZ-PC - ColetaDados.csv",
  header = T
)
...
df_names <- df_names |>
  ...
  mutate(
    geno = factor(if_else(
      df_names$generation %in% c("F1", "P1", "P2"),
...
df_pheno <- df_pheno |>
  as_tibble() |>
  mutate(
    Acamamento = case_when(
      Acamamento == "S" ~ 1,
      Acamamento == "N" ~ 0
    )
  ) |>
  mutate(
    Indeterminada = case_when(
      Indeterminada == "S" ~ 1,
      Indeterminada == "N" ~ 0
    )
  ) |>
...
df_pheno <- cbind(df_names, df_pheno) |>
  ...
  mutate(
    leaf_area = (0.347 * (leaf_length_cm * side_leaflet_length_cm * 2) - 10.7) /
      100
  ) |>
  ...
  mutate(leaf_color_C = sqrt(leaf_color_a^2 + leaf_color_b^2)) |>
  mutate(
    leaf_color_H = atan(leaf_color_b / leaf_color_a) *
      180 /
      pi +
      ifelse(leaf_color_a >= 0, 0, 180)
  )
...
df_dna <- df_dna |>
  mutate(
    electrophoresis_gel_quality = factor(
      electrophoresis_gel_quality,
...
# traits <- data.frame(abrv = c("nfh"),
#                      trait = c("ripe_fruits_harvested"),
#                      name = c("Number of fruits harvested"),
#                      fam = c(3),
#                      mod = c(12))
traits <- read.csv("traitsGLMMres.csv", header = T)

# x <- acomp(df_yield |> select(all_of(traits$trait)))
# x <- df_pheno |> select(all_of(trait))
# pcx <- princomp(x)
# sum(pcx$sdev[1:2]^2)/sum(pcx$sdev^2)

# pheno_fa <- factanal(na.omit(df_pheno |> select(all_of(traits$trait))+0.00001), factors = 1)
# pheno_fa

save(df_names, df_pheno, traits, file = "./LeafColorData.RData")
# save(df_names, df_yield, traits, file = "./NumbFruits.RData")
# save(pcx, irrig.idw, file = "./PlotData.RData")

# find <- df_pheno |>
#   select(c(colnames(df_names), "height_to_det", "internode_length", everything())) |>
#   filter(generation == "F2", internode_length <= 8, height_to_det >= 1.5) |>
#   arrange(internode_length, desc(height_to_det))
